...
namespace Resiliency
{
    ...
    public class PloyPolicyHandler : IPolyDbPolicyHandler, IPloyHttpPolicyHandler
    {
        ...
        public PloyPolicyHandler(IApiRequestHandler reqHandler, IConfiguration configuration)
        {
            ...
            _policyWrapDb = Policy.WrapAsync(retry, breaker);
        ...
        public IAsyncPolicy<HttpResponseMessage> GetHttpFallbackPolicy()
        {
            return Policy.HandleResult<HttpResponseMessage>(r => !r.IsSuccessStatusCode)
                .FallbackAsync(fallbackAction: (cancleTkn, task) =>
                {
                    _reqHandler.RaiseBusinessException("BaseFacade", "BaseFacade", "Reliency Error: fallback activated");
                    return Task.FromResult((new HttpResponseMessage(HttpStatusCode.NotFound)
                    {
                        Content = new StringContent("Reliency Error: fallback activated")
                    }));
                }
                , onFallbackAsync: (result, context) =>
                Task.Run(() => _reqHandler.RaiseBusinessException
                ("BaseFacade", "BaseFacade"
                , $"Fallback of Fallback {context.PolicyKey} at {context.OperationKey}: fallback value substituted, due to: {result.Exception}.")));
        }
    }
}
